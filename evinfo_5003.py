...
class Blockchain:
    ...
    def add_info(self, vin, cost, sold):
        # the last two parameters: USD cost per kWh, kWh sold
        self.info.append({'vin': vin,
                          'cost': cost,
                          'sold': sold})
...
def mine_block():
    ...
    #private blockchain, so no prize
    #blockchain.add_transaction(sender = node_address, receiver = 'Lucio', amount = 1)
...
def add_info():
    ...
    info_keys = ['vin', 'cost', 'sold'] # to make sure that the input is valid
    if not all (key in json for key in info_keys):
        return 'Some fields are missing.', 400 # bad request
    index = blockchain.add_info(json['vin'], json['cost'], json['sold'])
    response = {'message': f'This information will be added to {index}.'}
...
def connect_nodes():
    ...
    response = {'message': 'All nodes are connected. The EVinfo Blockchain now contains:',
